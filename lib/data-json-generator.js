...
module.exports = class DataJsonGenerator {
  ...
  static makePartsVariations(data, blockName) {
    return Promise.resolve(module.exports.makePartsVariationsSync(data, blockName));
  }

  static makePartsVariationsSync(data, blockName) {
    if(!data['defaults']['config']['partsVariation']) return {'partsVariations': ''};

    const partsVariationSize = PARTS_VARIATIONS[blockName] || data['defaults']['config']['partsVariation'].length;

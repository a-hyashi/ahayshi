...
module.exports = class DataJsonGenerator {
  ...
  static makePartsVariationsSync(data) {
    ...
    const partsVariationsValue = data['defaults']['config']['partsVariation'].reduce((pre, _variation, index) => {
      let str = '';
      if(index === 0) {
        str = '';
      } else {
  ...
  static makeOtherParametersSync(data, withoutPV = true) {
    ...
    const parameterNames = Object.keys(dataSchema.properties.data.properties).filter(name => {

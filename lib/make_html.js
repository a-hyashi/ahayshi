...
const sideMenu001 = require('./each_part/sideMenu001.json');
const breadcrumbs001 = require('./each_part/breadcrumbs001.json');
      ...
      partsVariations.map(async partsVariation => {
        ...
        if (filename.match(/^151_.*_sideMenu_a00_001$/)) {
          for (const lv of [1, 2, 3]) {
            let _html = html.replace('[sidemenu:0]', sideMenu001["htmlLv" + lv]);
            let _filename = filename.replace('a00_001', `f28_00${lv}`);
            let _outputfilePath = path.join(outputDir, _filename + '_' + this.paddingZero(partsVariation ? partsVariation : '1') + '.html');

...
module.exports = class BuddyPartsFunctions {
  ...
  static hasFunction(_data, partsCategory, functionId) {
    if(!pf[functionId]['testCase']) return false;

    return Object.keys(pf[functionId]['testCase']).some(testBlockKey => {
      return Object.keys(pf[functionId]['testCase'][testBlockKey]).some(testCaseKey => {
        return pf[functionId]['testCase'][testBlockKey][testCaseKey]['target'].includes(partsCategory);
      });
    });
  }

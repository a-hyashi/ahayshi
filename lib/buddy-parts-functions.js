...
module.exports = class BuddyPartsFunctions {
   ...
   */
  static hasFunction(_data, blockName, functionId) {
    ...
    return pf[functionId]['specifications'].some(specification => {
      return specification['testCases'].some(testCase => {
        return testCase['blocks'].includes(blockName);
      });
    });
  }
   ...
   * 有効な機能IDの一覧を返す
   */
  static targetFunctionIds(data, blockName) {
    return Object.keys(pf).filter((functionId) => {
      return module.exports.hasFunction(data, blockName, functionId);
    });
  }

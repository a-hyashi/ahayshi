...
const pf = require('./btool-settings/buddy-parts-functions.json');
...
module.exports = class BuddyPartsFunctions {
   ...
   */
  static hasFunction(_data, partsCategory, functionId) {
    if(!pf[functionId]['specifications']) return false;

    return pf[functionId]['specifications'].some(specification => {
      return specification['testCases'].some(testCase => {
        return testCase['blocks'].includes(partsCategory);
      });
    });
  }
   ...
   * 有効な機能IDの一覧を返す
   */
  static targetFunctionIds(data, partsCategory) {
    return Object.keys(pf).filter((functionId) => {
      return module.exports.hasFunction(data, partsCategory, functionId);
    });
  }
 ...
 * @param {*} config
 */
const forIconTypeWithSize = (data, pf, _config) => {
  let _labelsArray = [];

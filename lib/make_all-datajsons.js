...
let makeAllDatajsonRun = async (templateName, templateDir, outputFileDir, outputFileName, partsCategory) => {
  ...
  const partsData = await utils.readJsonFile(filepath).catch (err => {
    // テーマディレクトリにjsonがない場合にcommonを参照する
    console.log(`use common json: ${templateName}`);
    utils.readJsonFile(path.join(config.common_templates_dir, templateName, metaFileName)).catch(err => {

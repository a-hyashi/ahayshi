...
module.exports.makeUnitTestFiles = async (htmlDir, styleguideDir) => {
...
const makeAigisMDFile = async (htmlDirName, htmlDirPath, destPath) => {
  ...
  for(const functionKey of Object.keys(functions)) {
    const unittestAigisMarkdown = createUnittestMD(functionKey, htmlDirName);
...
const makeLogin = async (styleguideDir) => {
  const unittestAigisMarkdown = createUnittestMD('a00', '212_login');
...
const makeSmartphoneAssets0 = async (styleguideDir) => {
  const unittestAigisMarkdown = createUnittestMD('a00', '214_smartphoneAssets');
...
const makeSmartphoneAssets1 = async (styleguideDir) => {
  const unittestAigisMarkdown = createUnittestMD('a01', '214_smartphoneAssets');
...
const makeSmartphoneAssets2 = async (styleguideDir) => {
  const unittestAigisMarkdown = createUnittestMD('a02', '214_smartphoneAssets');
...
const createUnittestMD = (commentBlockCategory, htmlDirName) => {

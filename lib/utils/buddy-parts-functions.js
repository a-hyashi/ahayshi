...
const forSetStrings = (data, pf, config) => {
  ...
  const _labelstr = Object.keys(_properties).reduce((pre, key) => {
    if(STR_VALUE_TYPE[key] !== undefined) {
      return pre ? pre : STR_VALUE_TYPE[key] ? STR_VALUE_TYPE[key] : "";
    }else{
      return pre;
    }
  }, "");
...
const MIN_TEXT = '';
const MAX_TEXT = 'あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゃゆゅよらりるれろわ・を・んアイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤャユュヨララリルレロワ・ヲ・ンあいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゃゆゅよらりるれろわ・を・んアイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤャユュヨララリルレロワ・ヲ・ン';
const RTE_TEXTS = {
 ...
 * @param {*} after
 */
const makeMinData = (data, keys, _config, label, after) => {
  let _data = _.cloneDeep(data);
  for(const contentKey of keys) {
    if(_data.data.contents[contentKey].text === undefined) continue;
    _data.data.contents[contentKey].text = after;
  }
  return { [label]: _data };
}
 ...
 * @param {*} afterP
 */
const makeMaxData = (data, keys, config, label, after, afterP) => {
  let _data = _.cloneDeep(data);
  for(const contentKey of keys) {
    if(_data.data.contents[contentKey].text === undefined) continue;
    _data.data.contents[contentKey].text = (config.defaults.contents[contentKey].meta.type === 'wysiwyg' ? afterP : after);
  }
  return { [label]: _data };
}
...
const makeRTEData = (data, keys, config, label, rteKey, after, afterP) => {
  ...
  let rteFlag = false;
  for(const contentKey of keys) {
    if(_data.data.contents[contentKey].text !== undefined &&
        TYPE_SETTINGS[config.defaults.contents[contentKey].meta.type] !== undefined) {
      if(TYPE_SETTINGS[config.defaults.contents[contentKey].meta.type].includes(rteKey)) {
        _data.data.contents[contentKey].text = isWysiwygText(config.defaults.contents[contentKey].meta.type, rteKey) ? afterP : after;
        rteFlag = true;
      } else {
        _data.data.contents[contentKey].text = EXCLUDED
      }
    }
  }
...
const forText = (data, pf, config) => {
  ...
  _datas.push(makeMinData(data, _keys, config, '最小文字数', MIN_TEXT));
  if(isTemplate(data.templateDir)) {
    _datas.push(makeMinData(data, _keys, config, '最小文字数（テンプレート版）', TEMPLATE[data.templateDir].replace(/\[innerText\]/g, MIN_TEXT)));
  }
  _datas.push(makeMaxData(data, _keys, config, '最大文字数', MAX_TEXT, `<p>${MAX_TEXT}</p>`));
  if(isTemplate(data.templateDir)) {
    _datas.push(makeMaxData( data, _keys, config, '最大文字数（テンプレート版）', TEMPLATE[data.templateDir].replace(/\[innerText\]/g, MAX_TEXT), TEMPLATE[data.templateDir].replace(/\[innerText\]/g, `<p>${MAX_TEXT}</p>`)));
  }
  for(const _rteKey of Object.keys(RTE_TEXTS)) {
    const _RTEData = makeRTEData(data, _keys, config, 'RTE機能', _rteKey, RTE_TEXTS[_rteKey], `<p>${RTE_TEXTS[_rteKey]}</p>`);
    if(_RTEData) {
      _datas.push(_RTEData);
    }
    if(isTemplate(data.templateDir)) {
      if(['navList', 'footerUnit-navList', 'sideBoxList'].includes(data.templateDir) && ['ol', 'ul'].includes(_rteKey)) continue;
      const _after = TEMPLATE[data.templateDir].replace(/\[innerText\]/g, RTE_TEXTS[_rteKey]);
      const _RTETemplateData = makeRTEData(data, _keys, config, 'RTE機能（テンプレート版）', _rteKey, _after, _after);
      if(_RTETemplateData) {
        _datas.push(_RTETemplateData);
      }
    }
  }
  return _datas;
...
const forHtml = (data, pf, config) => {
  ...
  for(const _pattern of ['GoogleMap-200', 'GoogleMap-600', 'GoogleMap-1200', 'Youtube', 'Facebook', 'Twitter']) {
    _htmlPattern[_pattern] = fs.readFileSync(`./lib/each_part/${_pattern}.html`, 'utf-8');
  }

...
let forHtml = (data, pf, config)=>{
  ...
  _htmlPattern['Facebook'] = fs.readFileSync('./lib/utils/facebook.html', 'utf-8');
  _htmlPattern['Twitter'] = fs.readFileSync('./lib/utils/twitter.html', 'utf-8');

  let _dataArray = [];
  for(let _key of Object.keys(_htmlPattern)) {
    let _dataobj = {};
...
let forBackgroundImage = (data, pf, config) => {
  let _properties = pf.properties;
  let _data = _.cloneDeep(data);
  let _labelstr = '背景画像の表示想定';

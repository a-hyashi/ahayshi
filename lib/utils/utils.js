...
const config = require('../btool-settings/common_config.json');
    ...
    return (templateId) => {
      if(templateId in templates){
        ...
        try {
          ...
          // テーマディレクトリにtemplateがない場合にcommonを参照する
          console.log(`use common template: ${templateId}`);
          var template = fs.readFileSync(path.join(config.common_templates_dir, templateId + '/template.html'), 'utf-8');
        }
